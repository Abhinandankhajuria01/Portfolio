...
export default function Timeline() {
  ...
  useEffect(() => {
    ...
    // Scroll-driven item reveal
    itemsRef.current.forEach((item, idx) => {
      if (!item) return;
      gsap.fromTo(item,
        { opacity: 0.2, x: idx % 2 === 0 ? -40 : 40, scale: 0.9 },
        {
          opacity: 1,
          x: 0,
          scale: 1,
    ...
    // Highlight dots as the timeline line reaches them
    if (containerRef.current) {
      const dots = containerRef.current.querySelectorAll('.timeline-dot');
      dots.forEach((dot) => {
        gsap.fromTo(dot,
          { scale: 1, backgroundColor: "#000", borderColor: "#fff", boxShadow: "0 0 0px transparent" },
          {
            scale: 1.5,
  ...
  return (
    <section id="timeline" className="py-16 md:py-32 px-6 bg-black text-white border-t border-white/10">
        ...
        <div className="space-y-6 md:space-y-12">
          {milestones.map((milestone, idx) => {
            ...
            return (
              ...
              >
                {/* Center Dot */}
                <div className="timeline-dot absolute left-4 md:left-1/2 w-3 h-3 bg-black border-2 border-white -translate-x-[5px] md:-translate-x-1/2 z-10 rounded-full"></div>
                ...
                <div className={`w-full md:w-1/2 pl-10 md:pl-0 ${isEven ? 'md:pr-12 text-left md:text-right' : 'md:pl-12 text-left'}`}>
                  <motion.div 
                    whileHover={{ scale: 1.04, x: isEven ? -6 : 6, borderColor: "#00f0ff", boxShadow: "0 10px 30px rgba(0, 240, 255, 0.15)" }}
                    whileTap={{ scale: 0.96 }}
                    transition={{ type: "spring", stiffness: 400, damping: 17 }}
                    className="wireframe-card p-5 md:p-8 group cursor-pointer"
                  >
                    <h3 className="text-base sm:text-lg font-black uppercase tracking-wider mb-2 text-white">{milestone.title}</h3>
